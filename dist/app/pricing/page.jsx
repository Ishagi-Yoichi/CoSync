...
export default function PricingPage() {
    return (<main className="premium-shell relative min-h-screen overflow-hidden px-4 py-10 md:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          {plans.map((plan, index) => (<motion.section key={plan.name} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.45, delay: index * 0.08 }} className={`premium-panel relative rounded-[32px] p-7 ${plan.featured ? "premium-glow-border bg-[rgba(12,22,32,0.9)]" : ""}`}>
              {plan.featured ? (<div className="absolute right-6 top-6 inline-flex items-center gap-2 rounded-full bg-white/8 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-[#67e8c8]">
              ...
              <div className="mt-6 space-y-3">
                {plan.features.map((feature) => (<div key={feature} className="flex items-start gap-3 rounded-2xl border border-white/8 bg-white/[0.035] px-4 py-3">
              ...
              <Link href={plan.href} className={`mt-8 inline-flex w-full items-center justify-center rounded-full px-5 py-3 font-semibold transition-transform hover:-translate-y-0.5 ${plan.featured
