...
function SignInForm() {
    ...
    async function handleSubmit(e) {
        ...
        if (res === null || res === void 0 ? void 0 : res.error) {
            ...
            router.push((res === null || res === void 0 ? void 0 : res.url) || callbackUrl || '/');
        }
    }
    return (<main className="premium-shell relative min-h-screen overflow-hidden px-4 py-8 md:px-8 md:py-10">
      <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.92fr_1.08fr]">
        ...
        ].map((item) => (<div key={item} className="rounded-[24px] border border-white/10 bg-white/[0.04] px-4 py-4 text-sm leading-6 text-slate-200">
                {item}
              </div>))}
          </div>
        </motion.section>
        ...
        <motion.section initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.1 }} className="premium-panel-strong premium-glow-border rounded-[34px] p-6 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <label className="block">
              ...
              <input type="email" className="premium-input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com" required/>
            </label>
            ...
            <label className="block">
              ...
              <input type="password" className="premium-input" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" required/>
            </label>

            {error ? <p className="text-sm text-rose-300">{error}</p> : null}

            <button type="submit" className="premium-button premium-button-primary inline-flex w-full items-center justify-center gap-2">
              Sign In
              <IconArrowRight className="h-4 w-4"/>
            </button>

            <button type="button" onClick={() => signIn('google', { callbackUrl: '/' })} className="premium-button premium-button-secondary inline-flex w-full items-center justify-center gap-3">
