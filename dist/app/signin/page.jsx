    ...
    return (<div className="h-screen flex justify-center items-center bg-pink-50">
      <div className="block max-w-sm p-6 bg-white border border-pink-200 rounded-2xl shadow">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
          <label>
            Email:
            <input type="email" className="w-full mt-1 p-2 border rounded" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nik22@gmail.com" required/>
          </label>

          <label>
            Password:
            <input type="password" className="w-full mt-1 p-2 border rounded" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required/>
          </label>
