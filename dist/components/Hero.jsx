...
import { IconArrowRight, IconBroadcast, IconDeviceLaptop, IconMicrophone2, IconRefresh, IconUsersGroup, IconWaveSine, } from "@tabler/icons-react";
...
export default function Hero() {
    ...
    return (<main className="relative mx-auto flex max-w-7xl flex-col gap-14 px-4 pb-20 pt-10 md:px-8 md:pb-28 md:pt-14">
      <section className="grid items-center gap-10 lg:grid-cols-[1.2fr_0.9fr]">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, ease: "easeOut" }} className="relative z-10">
          ...
          <div className="mt-9 flex flex-col gap-4 sm:flex-row">
            <button className="premium-button premium-button-primary inline-flex items-center justify-center gap-2" onClick={() => router.push("/home")}>
              Launch Workspace
              <IconArrowRight className="h-4 w-4"/>
            </button>
            <Link href="/pricing" className="premium-button premium-button-secondary inline-flex items-center justify-center gap-2">
              Explore Plans
            </Link>
          </div>

          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            {productMetrics.map((metric) => (<div key={metric.label} className="premium-stat">
        ...
        <motion.div initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.12, ease: "easeOut" }} className="floating-card premium-panel-strong premium-glow-border relative overflow-hidden rounded-[34px] p-5 md:p-6">
          ...
          <div className="mt-5 rounded-[28px] border border-white/10 bg-[#09131f]/95 p-5">
            ...
            <div className="mt-5 grid gap-3">
              {workspaceSignals.map((signal, index) => (<div key={signal} className="flex items-center justify-between rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3">
      ...
      <section id="platform" className="grid gap-5 md:grid-cols-3">
        {valueCards.map(({ icon: Icon, title, body }, index) => (<motion.article key={title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.45, delay: index * 0.08 }} className="premium-panel rounded-[30px] p-6">
      ...
      <section id="use-cases" className="premium-panel rounded-[34px] p-6 md:p-8">
        ...
        ].map((item) => (<div key={item} className="rounded-[26px] border border-white/10 bg-white/[0.035] p-5">
