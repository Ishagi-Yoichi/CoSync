...
export default function Client({
    ...
    return (
        <div className={`rounded-[24px] border p-3 transition-all ${
            showSpeaking
            ...
            <div className="flex items-center gap-3">
                <div className="relative">
                    ...
                    <span className={`absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-[#09131f] ${
                        isAudioEnabled && !isMuted ? 'bg-emerald-300' : 'bg-slate-500'
                    }`} />
                </div>
                ...
                <div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${
                    showSpeaking ? 'bg-emerald-300/15 text-emerald-200' : 'bg-white/6 text-slate-300'
                }`}>
                    {showSpeaking ? (
