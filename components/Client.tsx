...
export default function Client({
    ...
    return (
        <div className={`flex items-center gap-3 rounded-lg border p-3 transition-colors ${
            showSpeaking ? 'border-emerald-400 bg-emerald-500/10' : 'border-gray-700 bg-gray-700'
        }`}>
            ...
            {showSpeaking ? (
                <IconWaveSine className="h-5 w-5 text-emerald-300" />
            ) : isAudioEnabled && !isMuted ? (
                <IconMicrophone className="h-5 w-5 text-sky-300" />
            ) : (
                <IconMicrophoneOff className="h-5 w-5 text-gray-400" />
            )}
        </div>
    );
}
